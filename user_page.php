...
$sql = "SELECT id, f_name, l_name, passw_, role_id, photo FROM users WHERE id='$currentUser'";
$result = $conn->query($sql);

while($row = $result->fetch_assoc()) {
    $currentUserName =  $row["f_name"];
    $currentUserLastName = $row["l_name"];
    $currentUserRole = $row["role_id"];
    $currentUserPhoto = $row["photo"];
    $currentUserPass = $row["pass_"];
}
...
<?php 
    echo '<a href="' . htmlspecialchars("delete_user.php?id=" . urlencode($currentUser)) . '" class="btn btn-default">'. "Delete this user" . '</a>';
?>
    </br><a href="main_adm.php" class="btn btn-default">Back</a>
</body>
</html>